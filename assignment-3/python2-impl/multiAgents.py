...
class MinimaxAgent(MultiAgentSearchAgent):
    ...
    def minimax_multiagent(node, successor_gen, utility_func, terminal_func, depth, num_adversaries=1):
        ...
        for state, action in successor_gen(node.state, node.agent_index):
            ...
            # Evaluate successor
            MinimaxAgent.minimax_multiagent(
                v, successor_gen, utility_func, terminal_func, next_depth, num_adversaries)

        # Find best successor, inherit utility from the selected node
        best_successor = _min_or_max(
...
class AlphaBetaAgent(MultiAgentSearchAgent):
    ...
    def alphabeta_multiagent(node, successor_gen, utility_func, terminal_func, depth, num_adversaries=1):
        ...
        # Generate successor nodes
        for state, action in successor_gen(node.state, node.agent_index):
            v = MinimaxNode.create_alphabeta_node(
                state, action, next_agent, node.alpha, node.beta)
            node.successors.append(v)

            # Evaluate successor
            AlphaBetaAgent.alphabeta_multiagent(
                v, successor_gen, utility_func, terminal_func, next_depth, num_adversaries)
    ...
    def getAction(self, gameState):
        ...
        current_node = MinimaxNode(
            gameState, alpha=-float('inf'), beta=float('inf'))
